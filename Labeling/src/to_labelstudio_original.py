...
def generate_ls_json():
    ...
    for item in dataset:
        ...
        # Como usamos rutas absolutas y LS local file server, empaquetamos el path
        
        # En Label Studio puedes montar storage local que lee path directo usando "file:///..."
        # O sirviéndolos vía nginx/python server. Asumimos montaje "Local Storage"
        ls_tasks.append({
            "data": {
                "image": str(Path(item['file_path']).resolve())
            },
            "predictions": [{
